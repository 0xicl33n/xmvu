...
function buildProcessing( curElement ){        
  ...
  p.buildImageObject = function( obj ) {
    var pixels = obj.data;
  ...
  p.loadImage = function loadImage( file ) {
    ...
    var data = p.buildImageObject(imageData);
    data.img = img;
    return data;
  };
  ...
  p.get = function get( x, y ) {
    ...
    if ( !getLoaded ) {
      getLoaded = p.buildImageObject( curContext.getImageData(0, 0, p.width, p.height) );
    }
  ...
  p.loadPixels = function() {
    p.pixels = p.buildImageObject( curContext.getImageData(0, 0, p.width, p.height) ).pixels;
  };
